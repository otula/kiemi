...
import tuni.data.security.model.Definitions;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Override
 	protected void configure(HttpSecurity http) throws Exception {
		...
		http/*.requiresChannel().anyRequest().requiresSecure().and()*/.cors().and().csrf().disable().authorizeRequests().regexMatchers(HttpMethod.GET, "/files/\\d+").permitAll().antMatchers("/data/**", "/files/**").hasAuthority(Definitions.ROLE_USER).and().httpBasic();
 		//NOTE; csrf is disabled to allow easier testing of web pages, it should be enabled on production
		//http.csrf().disable().authorizeRequests().antMatchers("/data/**", "/files/**").hasRole("USER").and().httpBasic();
 	}
