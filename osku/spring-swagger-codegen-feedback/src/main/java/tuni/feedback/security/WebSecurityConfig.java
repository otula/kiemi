...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Override
 	protected void configure(HttpSecurity http) throws Exception {
		...
		http/*.requiresChannel().anyRequest().requiresSecure().and()*/.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.POST, "/users").permitAll().antMatchers("/questionares/**", "/locations/**", "/statistics/**", "/users/**").hasAnyAuthority(Definitions.ROLE_USER, Definitions.ROLE_ADMIN).and().httpBasic().and().logout().invalidateHttpSession(true).deleteCookies("JSESSIONID").logoutUrl("/user/login.html?logout=true");
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		dap.setPasswordEncoder(_userService.getEncoder());
