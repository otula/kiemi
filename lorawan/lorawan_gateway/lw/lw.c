...
{
    for (size_t j = 0; j < (sizeof(lw_node_maccmd_tab) / sizeof(lw_maccmd_len_t)); j++) {
...
{
    for (size_t j = 0; j < (sizeof(lw_server_maccmd_tab) / sizeof(lw_maccmd_len_t)); j++) {
...
int lw_mtype_msg_up(lw_frame_t *frame, lw_node_t *cur, uint8_t *msg, int len)
{
    ...
    if( len > (8 + 4 + foptslen) ){
        if( len == (8 + 4 + foptslen + 1) ){
            ...
            fprintf(stderr, "PORT (%d) PRESENT WITHOUT PAYLOAD", frame->pl.mac.fport);
        }else{
            frame->pl.mac.fport = msg[LW_DATA_OFF_FOPTS + foptslen];
...
{
    for(size_t i=0; i<sizeof(lw_region_tab)/sizeof(lw_region_tab[0]); i++){
